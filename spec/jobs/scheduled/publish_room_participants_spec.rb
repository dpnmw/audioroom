...
RSpec.describe Jobs::PublishRoomParticipants do
  ...
  it "publishes participants for rooms with active participants" do
    Audioroom::ParticipantTracker.add(room.id, user1.id)
    Audioroom::ParticipantTracker.add(room.id, user2.id)

    # Verify participants were added
    expect(Audioroom::ParticipantTracker.user_ids(room.id)).to contain_exactly(user1.id, user2.id)

    messages = MessageBus.track_publish { subject.execute({}) }

    room_messages = messages.select { |m| m.channel == Audioroom.room_channel(room.id) }
  ...
  it "reflects TTL-expired participants in broadcast" do
    ...
    Discourse.redis.srem(
      "#{Audioroom::ParticipantTracker::KEY_NAMESPACE}:#{room.id}:participants",
      user2.id,
    )
  ...
  it "handles rooms that no longer exist" do
    Audioroom::ParticipantTracker.add(99_999, user1.id)
